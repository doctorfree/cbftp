...
usage() {
  echo "Usage: sudo ./Install-bin.sh /path/to/cbftp_<version>-<release>.<arch>.<suffix>"
  echo "Where:"
  ...
  echo "https://github.com/doctorfree/cbftp/releases"
  exit 1
}
...
[ "${pkgname}" == "cbftp" ] || {
  ...
  printf "\n\tcbftp_<version>-<release>.<arch>.<suffix>\n"
  echo "Distribution archive filename ${ARCHIVE} does not begin with 'cbftp'"
  usage
}
...
echo "available at https://github.com/doctorfree/cbftp/releases"
echo ""
while true
do
